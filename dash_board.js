...
const showAllData =()=>{
    document.getElementById("dashboardContainerGraph").innerHTML = `<canvas id="myChart"></canvas>`
    ...
    //for total time per day in a week;
        ...
        document.getElementById("dashboardStatsUnique").innerHTML = `
        <div>   
            <span>Total Time</span>
            <h2 id="dashboardStatsTotalTime">${sumTotal} Min</h2>
        </div>
        <div>
            <span>Average Time</span>
            <h2 id="dashboardStatsAverageTime">${Math.floor(sumTotal/7) +` Min`}</h2>
        </div>
      
        `

        const dashboardReport = document.getElementById("dashboardStatsDetails");
...
const filterByProjectName = ()=>{
    ...
    if(selectedProject === ""){
        ...
        totalTime = dataCollector.reduce((acc,item) =>{
         ...
         document.getElementById("dashboardStatsTotalTime").innerHTML = totalTime +` Min`;
         document.getElementById("dashboardStatsAverageTime").innerHTML = Math.floor(totalTime/7) +` Min`;
